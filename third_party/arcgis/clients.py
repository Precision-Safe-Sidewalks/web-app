...
class ArcGISClient:
    ...
    def search(self, item_type=None, modified_range=None):
        ...
        if modified_range:
            start = int(1000 * modified_range[0].timestamp())
            end = int(1000 * modified_range[1].timestamp())
            query["filter"] = f"modified: [{start} TO {end}]"
